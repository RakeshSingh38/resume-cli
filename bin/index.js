...
const { parseFlags, getTerminalCaps, getWidth } = require('../src/utils/config');
...
const { commanderLib, chalk, inkLib, React, App, figletLib, gradientLib, clearLib, boxen } = require('../src/utils/deps');
...
function setupCLI() {
  ...
  program
    ...
    .option('-j, --projects [name]', 'Show projects (optionally filter by name/number)')
    ...
    .option('--save [filename]', 'Save resume to file (default: rakesh-resume.txt)')
    .option('--links [project]', 'Show project links (optionally filter by project name/number)')
    ...
    .option('--unicode', 'Force Unicode borders (overrides platform detection)')
  ...
  try {
    ...
    Object.entries(opts).forEach(([key, value]) => {
      if (value === true) {
        ...
        flags.push(`--${key.replace(/([A-Z])/g, '-$1').toLowerCase()}=${value}`);
      }
    });
 ...
 */
const getSection = (data, type) => data.sections.find(s => s.title.toLowerCase().includes(type));
...
const simpleCommands = {
  ...
  education: (data) => {
    ...
    section?.items?.forEach(item => {
      console.log(`${item.institution}${item.location ? ' | ' + item.location : ''}`);
      console.log(`${item.degree}${item.gpa || item.score ? ' • ' + (item.gpa || item.score) : ''}`);
  ...
  achievements: (data) => {
    ...
    section?.achievements?.forEach((ach, i) => 
      console.log(`${i + 1}. ${ach.name}${ach.date ? ' (' + ach.date + ')' : ''}`)
    );
  },
  ...
  projects: (data, query) => {
    ...
    if (query && query !== true && typeof query === 'string') {
      const project = section.projects.find((p, i) => 
        p.name.toLowerCase().includes(query.toLowerCase()) || (i + 1).toString() === query
      );
      ...
      if (project) {
        ...
        console.log(`Project "${query}" not found.\nAvailable projects:`);
        section.projects.forEach((p, i) => console.log(`${i + 1}. ${p.name}`));
      }
    } else {
  ...
  save: (data, filename) => {
    ...
    const outputFile = filename && filename !== true ? filename : 'rakesh-resume.txt';
...
function formatResume(data, colors) {
  ...
  data.sections?.forEach(section => {
    ...
    if (section.items) {
      section.items.forEach(item => {
        if (item.institution) {
          lines.push(`${item.institution}${item.location ? ' | ' + item.location : ''}${item.period ? ' (' + item.period + ')' : ''}`);
          if (item.degree) lines.push(`- ${item.degree}${item.gpa || item.score ? ' • ' + (item.gpa || item.score) : ''}`);
        }
      });
    } else if (section.categories) {
      section.categories.forEach(cat => lines.push(`${cat.name}: ${cat.skills.join(', ')}`));
    } else if (section.projects) {
      section.projects.forEach(proj => {
        lines.push(`${proj.name}${proj.subtitle ? ' – ' + proj.subtitle : ''}${proj.stack ? ' | ' + proj.stack.join(', ') : ''}`);
        proj.highlights?.forEach(h => lines.push(`- ${h}`));
      });
    } else if (section.achievements) {
      section.achievements.forEach(ach => lines.push(`- ${ach.name}${ach.date ? ' (' + ach.date + ')' : ''}`));
    }
...
function main() {
  ...
  if (flags.json) {
    ...
    if (flags.jsonPath) return console.log(JSON.stringify(getByPath(data, flags.jsonPath), null, 2));
  ...
  if (flags.jsonResume) {
    ...
    if (flags.printResume) return printValue(getByPath(jsonResume, flags.printResume));
    if (flags.jsonResumePath) return console.log(JSON.stringify(getByPath(jsonResume, flags.jsonResumePath), null, 2));
  ...
  const header = createHeader(data.name, { figletLib, gradientLib, clearLib, colors, caps, flags });
  const boxedContent = createBox(content, { boxen, unicode: caps.unicode, colorSupported: caps.color, contentWidth });
